...
# git-commit-id-plugin (useNativeGit=true) walks up to the monorepo .git - needs the git binary
...
# published to a reachable registry (same monorepo, no GHCR access) - install
# them into the local repo first. checks profile is CI-only.
