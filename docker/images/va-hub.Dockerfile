...
RUN mvn -f clns-acuity-va-security/pom.xml -B -pl domain,common,auditlogger,auth -am \
    ...
    && mvn -f clns-acuity-vahub/pom.xml -B -pl vahub -am -Pwebapp \
    -DskipTests -Dfindbugs.skip=true -Dcheckstyle.skip=true clean package \
    && cp clns-acuity-vahub/vahub/target/vahub-*.war /build/app.war
