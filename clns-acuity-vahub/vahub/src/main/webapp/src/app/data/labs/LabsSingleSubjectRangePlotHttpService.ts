...
import LabMeanRangeSelectionRequest = Request.LabMeanRangeSelectionRequest;

@Injectable()
export class LabsSingleSubjectRangePlotHttpService extends LabsHttpService {
    ...
    getPlotData(datasets: Dataset[],
        ...
        return this.http.post(getServerPath('labs', 'mean-range-plot'), JSON.stringify(postData))
            .map((data: any) => {
                return <List<IPlot>>fromJS(data.map((value: TrellisedRangePlot<Lab, LabGroupByOptions>) => {
                    return {
    ...
    getSelection(datasets: Dataset[],
        ...
        const postData: LabMeanRangeSelectionRequest = {
            datasets,
            populationFilters: this.singleSubjectPopulationFilter(),
            labsFilters: this.labsFiltersModel.transformFiltersToServer(),
            selection: {
