...
import AesPlotRequest = Request.AesPlotRequest;
...
@Injectable()
export class AesBarLineChartHttpService extends AesHttpService {
    ...
    getTrellisOptions(datasets: Dataset[],
    ...
    getXAxisOptions(currentDatasets: Dataset[]): Observable<DynamicAxis[]> {
        const path = getServerPath('aes', 'overtime-xaxis');
        const postData: any = {
            populationFilters: this.populationFiltersModel.transformFiltersToServer(),
            aesFilters: this.aesFiltersModel.transformFiltersToServer(),
    ...
    getPlotData(currentDatasets: Dataset[],
        ...
        const path = getServerPath('aes', 'overtime');
        ...
        return this.http.post(path, JSON.stringify(postData))
            .map((data: TrellisedOvertime<Ae, AeGroupByOptions>[]) => {
                return <List<IPlot>>fromJS(data.map((value: TrellisedOvertime<Ae, AeGroupByOptions>) => {
                    return {
