...
import {PopulationFiltersModel, CardiacFiltersModel} from '../../../../filters/dataTypes/module';
import {TrackData, TrackRequest} from '../IDataService';
import {BaseTrackDataService} from '../BaseTrackDataService';
import {SessionEventService} from '../../../../session/event/SessionEventService';
import {EcgTrackDataTransformer} from './EcgTrackDataTransformer';

@Injectable()
export class EcgTrackDataService extends BaseTrackDataService {

    constructor(protected http: HttpClient,
                private ecgTrackDataTransformer: EcgTrackDataTransformer,
    ...
    fetchDataForLevel1(request: TrackRequest): Observable<TrackData[]> {
        const path = getServerPath('timeline/ecg', 'summaries');
        const postData: any = this.getRequestPayload(request);
        return this.fetchTimelineTrackData(request, path, postData)
            .map((response) => {
                return this.ecgTrackDataTransformer.transformEcgSummaryTrackData(response);
            });
    }

    fetchDataForLevel2(request: TrackRequest): Observable<TrackData[]> {
        const path = getServerPath('timeline/ecg', 'details');
        const postData: any = this.getRequestPayload(request);
        return this.fetchTimelineTrackData(request, path, postData)
            .map((response) => {
                return this.ecgTrackDataTransformer.transformEcgDetailsTrackData(response);
            });
    }
