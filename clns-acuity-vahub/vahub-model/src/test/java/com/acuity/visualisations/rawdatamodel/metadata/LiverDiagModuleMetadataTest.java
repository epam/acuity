...
public class LiverDiagModuleMetadataTest {
    @InjectMocks
    private LiverDiagModuleMetadata moduleMetadata;
    @Mock
    private LiverDiagDatasetsDataProvider datasetsDataProvider;
    @Mock
    ...
    public void shouldGetMetadata() {
        ...
        when(datasetsDataProvider.loadData(any(Datasets.class))).thenReturn(Arrays.asList(
                new LiverDiag(LiverDiagRaw.builder().build(), Subject.builder().build()),
                new LiverDiag(LiverDiagRaw.builder().build(), Subject.builder().build()),
                new LiverDiag(LiverDiagRaw.builder().build(), Subject.builder().build())));
        ...
        //Then
        softly.assertThat(result.getKey()).isEqualTo("liverDiag");
        String json = result.build();
        softly.assertThat(json).contains("\"count\": 3");
        softly.assertThat(json).contains("\"hasData\": true");
        assertThatJson(json).node("liverDiag.detailsOnDemandColumns").isEqualTo(Arrays.asList("column1", "column2"));
    }
}
