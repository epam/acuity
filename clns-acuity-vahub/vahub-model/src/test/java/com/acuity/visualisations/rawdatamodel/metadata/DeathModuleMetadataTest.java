...
public class DeathModuleMetadataTest {
    @InjectMocks
    private DeathModuleMetadata moduleMetadata;
    @Mock
    private DeathDatasetsDataProvider datasetsDataProvider;
    @Mock
    ...
    public void shouldGetMetadata() {
        ...
        when(datasetsDataProvider.loadData(any(Datasets.class))).thenReturn(Arrays.asList(
                new Death(DeathRaw.builder().build(), Subject.builder().build()),
                new Death(DeathRaw.builder().build(), Subject.builder().build()),
                new Death(DeathRaw.builder().build(), Subject.builder().build())));
        ...
        //Then
        softly.assertThat(result.getKey()).isEqualTo("death");
        String json = result.build();
        softly.assertThat(json).contains("\"count\": 3");
        softly.assertThat(json).contains("\"hasData\": true");
        assertThatJson(json).node("death.detailsOnDemandColumns").isEqualTo(Arrays.asList("column1", "column2"));
    }
}
