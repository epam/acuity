...
public class LiverRiskModuleMetadataTest {
    @InjectMocks
    private LiverRiskModuleMetadata moduleMetadata;
    @Mock
    private LiverRiskDatasetDataProvider datasetsDataProvider;
    @Mock
    ...
    public void shouldGetMetadata() {
        ...
        when(datasetsDataProvider.loadData(any(Datasets.class))).thenReturn(Arrays.asList(
                new LiverRisk(LiverRiskRaw.builder().build(), Subject.builder().build()),
                new LiverRisk(LiverRiskRaw.builder().build(), Subject.builder().build()),
                new LiverRisk(LiverRiskRaw.builder().build(), Subject.builder().build())));
        ...
        //Then
        softly.assertThat(result.getKey()).isEqualTo("liverRisk");
        String json = result.build();
        softly.assertThat(json).contains("\"count\": 3");
        softly.assertThat(json).contains("\"hasData\": true");
        assertThatJson(json).node("liverRisk.detailsOnDemandColumns").isEqualTo(Arrays.asList("column1", "column2"));
    }
}
