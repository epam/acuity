...
#         
    def test_createPatient(self):
        cd = clinicaldata.ClinicalData("ClinicalCUP0010001-nodate.csv", "uomdevsqlserveret.database.windows.net", "et_python_process@uomdevsqlserveret.database.windows.net", "Funvax41!", "uom_dev_etarget_db", "uomdevstorageet", "GIGjJSDgeMf3bS8EJVclOHEPOz8/Dxan6wWYjJa73MSLT5IIkpo81ZHPy5cncvMWrVuuIbiKqEMvwYO7KvpKDw==", 'test')
        pd = cd.getPatientDetails(cd.readLine(2))
        print(pd['patientNumber'])
        patient_id=cd.createPatient(pd)
        print(str(patient_id))
        self.assertTrue(patient_id>0)
        print('end test_createPatient\n')
#       
#       
